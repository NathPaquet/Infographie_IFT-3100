...
vec3 brdf_cook_torrance()
{
  ...
  float diffuse_reflection = max(dot(new_normal, l), 0.0);

  // calculer la distribution des microfacettes
  float d = trowbridge_reitz(new_normal, h, roughness);

  // calculer la fonction géométrique
  float g = smith(new_normal, l, v, roughness);
  ...
  float coor_torrance_denominator = 4.0 * max(dot(new_normal, v), 0.0) * diffuse_reflection;
